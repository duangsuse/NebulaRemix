...
Shell不区分类型错误和语法错误、反人类的引号和$、给低频操作极短缩写的设计真的很蠢：

`if; then echo; fi` 不是语法而是两条/bin/命令，到处都是 `"$1 arg"  "$var"; k[1]=2; $k[1]`，各种无效的glob和eval般恶劣的展开，有效的却被挤出C位

sh里真正的接口是 $(), |, >> 和 echo;cat;tee; sed 空格数组。甚至连argv格式都未规定，Tab补齐本质上是n=1的N-gram但基本没人会写。sh读了app再读，用的方法可能都不同，根本没有协议意识

CLI本来是基于补齐、色彩、可远程可复现的交互式编程，是面向快捷键、文件的组合、ANSI鼠标，而不是一个到处echo改文件的脚本版overlayfs。

rg;fd 这些都是GUI选区模式也有的，而只能管道文本(aa -o | bb -in) 证明了无类型的问题

bb <(aa -o /dev/stdout) 证明了管道的设计并不走心，如果不用 -o 区分换行符流和hex流，aa会打乱你的终端，而最简单的浏览器都知道哪些【链接】会直接打开，哪些自动下载，bash不会。

在脚本里临时文件名无意义还影响SSD寿命，UNIX却鼓励md;cd完再rm一下，不md还会直接卡死你，防呆到看不懂气氛也是很【严谨】了。不编译也并非零摩擦。如果是我， 2&1>/dev/null 设计成 &|! 和 |! 的管道就行了，&| 还可以直接扩展成 &showErr| 和 &hideErr|

相对的，`$mapfile[./path]; set -- ${(s::)tsv}; $1; jq .id` 规避了awk/sed，是不错的特性
