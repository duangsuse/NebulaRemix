...
_if_cucupX_ 十一XノGLtE .varGITES
ON/NO/XX/dd, OK/KO/passOrLoop, m_oo0nBits区间框 (oo.dt 和inc点动框,TAU/s); nd[[3,inc(索引)]]

JS/XML/JsonfTTP DMM(json""和jenc,toml2表,vtt录制,mdd,htmb)
OS43 -sh_iox -withEval模块 -ufotwk交互 -SQRUT -URow@URoot

Str. AZ正择/AHex/YmdHMS/ItemU,join   -PahQL AZ.eat/双懒Bisect -BYGRPxBB/BBop  bracey{TDOP Word} BS-VOcXS链 .:[-=~>]:{括需块}:.
f`str`,az组(after_pass/B4), slang,strfmt, saytest.dDTFM, At,Sum.KV流(4.3ATs), sh
OS  uname去前缀 
CPU add1 jobN atom1 UI UX UGC VFX AFX

Row nextID0(x) var/typeOf ioFmt/cpio.9p  og_ivStack
Row1~3 D1~D4分量同算 (Col)N0~N5免前缀
Col. bind fill pipe Neg override/oldFun  Yup idIsSelf  -your _404(馈硬可返{}) FN1~5(不默认首参主语) “十全十美”

/* 
----
阴阳五行 let/each或自身+filterMap/firstIndex  
拟态匹等 sikfane名即类型 ColXXIA考下Quine_三定律 TDDgc副作用根剪枝/sndo_fakmut 回填宏和重录#pb,#@1
三态四技 海陆空多态  组合eDSL/Ho 拆解Proxy 转换Seer 联合CTAG

创编六艺 构造枝 加造链(new_class) 加参(kwenv,inner类覆写) 三段构造 三字面四层变 三表格器

跨端七彩虹 py,js(esm,lib)  ps1,jsh  rs,go(前俩转AOT)  zigCbroLua S3RPA
  COM,gdbus,Apple/AHK跨脚本  跨端 RwSeer/spgjiaVM,Loadump,post_onMsg 管线
  S3RPA(SQL,ShaderToy,Scratchy探索式, Ruby,PHP,VBA)

braceyTDOP 支持 (x:xs),(=>)的右结合，为流跟踪[缩进栈]- 类似于解析(not,-)时可指定stop级别-默认';'。
sepRightFirst(';',[+-]) 自动插入优先级，(as Type)(? A:B)等运算作为后缀/调用链，即unifix多项折叠。 eat“双懒”基于双删丢行反冒泡

调用无形
  Web3 stdpayv拍物多签茧  pOTP链号登录圈  bioSMS端端私聊/隔空放币即拍即付 Web3Hook三前后端/pjoPager二层两栏两词列表详情协议
  
  单一unique 注入inject 可为allow，每个词都有料： types as const, operator/infix, derive宏。比如 `[unique T]=typealias T,  [[allow Copy Eq]]`
  ^^ allow rw/arg 形变方向，可 .[[T trust_me] As] 伪子类强转

[-] 波折号此's四类参 零参句尾调用 (An){u}.:免遮盖管道(首参数注入)

sh_iox  [TOML2/json""L],xBB_pickr :(IWEXpls:n=.ftyp="msg"):  :. pjo:Pager(vv),[[!pic](linkBtn)] cls_gc^CRXpx 界面色和#!/env文件内插  of. tg,jup,grok
    

*/

/*
快速调试 d=is,s,模块eval e=$0 dm=路径搜索   ev框架监听器API-执行  dt=ns/cu单位 numpath-treemap
ui tap和kbd快捷键(CAS-/^)
mountFetch默认离线  
OS. chop(cue-refine) async/chhome(moonchor) 进程转云函数
varGITES bufsz=-msTime/cu,av,dpr  一等微分
rwxi IO  ur.filesWalk,uh.dirsMeta

Str 元字符 *+?()[]\
*+ 只在()右侧或内中生效
· 替代了空格(即 [s]+)， ?var(P) 模式表达?<var>或(?>P)
(**) 视 //p 确定是[^/]或[S]
...
左右匹等
new与inc赋值同义： `new i == 0; while(i<a.n){new i (i+1)}` ，`new h5vid {iTime=u=>u+1.5}; inc(h5aud ({iTime:u=>nextChap(u), ..}), )`
可变性被隔离： incPub(可在this或闭包共享旧变量) inc{As} incInto(new->never)  Loop式调用和DFS
变量树 Arg(ArgTree半冻结/AutoBox免装箱/inHole多点等值) VarTree_变更量 Row首参多态(文件内无法sealed)；调用树 LetsTree(Col粘贴,forif特化) Quine_


行列多动态(RCH polymo.) 是由行的形状解构、列的传递耦合正交编织的类型多态：everything is Row(Object), or.. VarTree_, ArgTree, Arg, or: AutoBox & inHole(Rw/Ho) value
行是可展开的浅层“(...)树枝”。没错，树枝可为(多种键值)，比如H5 `If片段=文本 | ((粗|斜|行) If片段...)` 键名 `str|(tag subs)`，但枝总是可平铺(字串化)且非null。只有Row和"ItXXX"类型有可能在文件外被extends破坏。
列(Fun/Hole)是含多点等值的“树枝”，用于重复出现的计算。只含 Arg|AutoBox|inHole 才是不可变的Arg，否则是ArgTree元组，以此类推到 java/js Row。 
修改“不可变”的Rw[]参数并访问“深拷贝”树的效率最接近Arena。`Ho<T>` 比 `Rw<T>` 更接近 Vue ref(v)，它可表达 List,Dict,IfTypes。
- never/cue资源管理  局部据柄以cue(cu)和def先后绑定，以never(cu)移走。包含 ItClose(u.cuXXX) 的T必须命名为 T_，含 unique_ptr(.ccXXX) “外键”的同理
  - vs Rust：写独占但读写不互锁、RAII风格的move语意。[_if_cucupX_] 还包含： .up|owner,arc,cu/cc 即 弱/跨线程/单容器指针。
- 易变后缀退化  除了 never,inc, incInto(js) ，任何赋值和传参都自动去掉 T_ 的尾部。这涵盖了集合类，参见 [Austral一次性使用规则]
- 易变和空值优化  只有结构体、参数里可为空。never会用null删掉原位置，KO""简单返null并设置(Row.noMeansErr)。NO返回的实现为Optional，NaN在ints里实现为MIN

强大的结构性和静态断言、高度动态的数据流和列表处理、框架内一切的跨语跨端、Web领域最先进的心智模型
- 换首律：追溯调用 QuineOp_() 的函数名，替换 src/ 下的首个"()"区间为"&&"+首次输出。comptime可用 `全局=lityped.run((x,y,)=>Q_(xyTreeDSL..))`，.Ret_("a z") 用于az分词等简单常量折叠。被替换行不得含运算符，调用窥硬的lib函数不得无测试覆盖。
- 内插律：`.Quine_($=>{})` 内 paste($=>[()=>op,])[i] 首次引用插入变量名(故得名LetsBlock/lb)，而魔法在于op块中 `(cell值JSON或名)` 内联
  - lb[i]可用于 `forif,$.T.callFuns, $[1]=For语句的var, $[0](If句的OK)`，c=paste({op(){}}) 将内联箭头等(cell=c.op)
- 实用律：窥硬即展开/折叠/内嵌，如 simplifyAdd, sexpXML, swapAB 的惯用法： `OK(paste([_=>A.v+B.v,_=>B.v==0?A:pass, _=>A.十(B)]))` 在参数常量时优化，sexpXML在多个lb里将attr上树并append所有节点，swap利用 `lb_inc_save1;$[1].As($[2])` 来将lb符号作为左值

JSON_ft名即旗表
- 禁止键名 "", "\n", "-XX", 另见 [_if_cucupX_];   "*_" (允许v空), "if_*" (多态项), "_*" (v被强制dumps)
- 只允许包含>1项的{}, ==1 项的[x]以 {"\n":[x,"\t1OK"]} 保存而 [0],[{}].. 为强类型空数组
...
五答是为了组合出主流语言的基石——Let块/基本块内的【字相等可组】：字面值、相对值(即语法/`ast=>域=>值`)、等位值(多点等值/`x+x`)、可选值(&&||和null链)、组合字面值(JSON.[]{}+proto链)。

“值”可传入为栏目，比如“有值的”函数可深先遍历为等位值： `f1(f(x)).f2(3) -> [A=f运行时链接好(moduleGlobal等解析期查好了域['x']), B=f1(A), C=B['f2'].call(B,3), Ret(C)]`，这样理解传参，await状态机和 `(1==1? f:null)?.(x) ?? x` 跳转容易实现。

lambda演算一般结合SExp的嵌套[Any List]树与链表实现并demo，在单步执行(eval-visit)时而非提前替换 `a,x,(+)` 为字面定义。按深先遍历，这也可用RPN栈或SSA树状图/块状图表示。 `..'f2'.call(B,3)` 也是JVM的拿手好戏，等效if(instanceof)check。
...
$3T 类型名禁下划： $_Int_Ln$  _Z2As4pass _Z2As$_Str$_Int_Ln$N2$

IOSEE函数侧写法：
IO参数和语法限制了框架的适用范围，比如bash/http/py都使用kwarg和标准单参数、管道(py的this链,fetch的Body)，但语法的长短和搭配差异巨大。
State是“F5刷新不掉的报错”，Effects是“函数卡死也阻挡不了的输出”，它单次调用生成多项，自然无法通过API契约来观测。良好的效应产生的多项比iter栈(async返回链表)快的多

IOSEE入门4例： EchoArgs, CounterDown, Checklist_LiveGrep, ClocMiB_ForkJoin
...
[JSON_ _flattbl] naming convention
...
IOSEE不是为复用计算树，做函数的替换、类型、等效性等推理，而是侧写其语意——Input/Output建模，State变量树，Effects/Ext/Enclose

- In是有侧重点的：bash按使用频率有 --kwarg={n,enum}, rm-rf“复选框”, (std)file, 输出点或键arg；py/http 却是 `def op(self, *任意arg,**kw)`，参数设计了框架/语法的适用范围
- Out需要优化/检查：输出值、Error和空结果、引用点(如深结构体和可变[])，包括最容易忽视的变量树底层实现(GC/Rc/Arena.FlatBuffer)
- State是“F5刷新不掉的报错”，也是函数对this以外var容器的“破坏性变更”，它并非缓存击穿的元凶（低效源于错误的隔离）
- Effects是“页面的阻塞挡不了的值”。效应器在单次Out效率太低时，单次调用生成多项，自然无法通过API契约来观测。
  - 虽然async的“协程=返回链表”基于yield流来链接，它也不是“效应器”。良好的效应生成的多项比iter栈上的快的多，它才是真正的多返回值。
  - 除了资源包与文件树(范围)的容器化，App体系更需要准确定义自身特性/领域的边界，通过对用户故事的感知能力尽量添加扩展点，从而以最小权限原则检查。
