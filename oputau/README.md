# SV-TGR：调声和鬼畜创作的简单工作流

> TGR: Track&Tick, Grouping, Retakes <mark>（音准卡点-填词-重录三步走）</mark>
...
1. 【带歌词的midi格式全曲】下载原曲经过人声分轨(Track)，导入SynthV，点右键自动扒谱（配合TGR重敲小片段-"Retick"）
...
`OpenUTAU>工具>Packages>game` 在音高上比SV还准，扒谱前需在SV检测曲速并在左上角填一下，XS同理，且中文分词比SV准太多。[每个网易号1天就有两次免费，不要浪费~](https://xstudio.music.163.com/#page_two)
...
翻唱快节奏热歌（APT. PPAP..）全自动扒谱撞南墙？想要中英日韩翻着调（修改歌词吐字字数）？TGR工作流让你【重新理解】钢琴卷帘！
...
ps. 有天资的示例： [直播唱Uptown FUNK](https://www.bilibili.com/video/BV1nQ4y1i7o2)、 [小学生使用合成器](https://www.bilibili.com/video/BV1mcDvBTE93)

可能纯填词都有天赋怪，没想到吧？就码个字的事…… [数码宝贝Butterfly改编](https://www.bilibili.com/video/BV1eNtieWEZH)
...
Alt+R 重敲所选音符
- 选中需重敲的部分按Alt+R，然后把走带拖到原唱开口前几秒，即刻弹出网页，空格开始为每个音高打轴。无选区则插入空音符组，以左键试音，右键+1条音高，d键删除
- 一指禅：S键设置右1音符的 on-off timing, A键放置休止符(f0-on-dur timing)，d从头再来。完成后按S重新弹奏，空格预览A完工
- 钢琴键网页总是用于从所选音符开始往后设置音高，w键播放暂停
    - Tips：您可以选择mp3来【免SV使用】Retick功能，完工一键导出 .mid,json （过程中不可退出网页或查看，只能从左向右添加音高组并打轴、拖拽进度条）
    - 👍可以与 OpenUTAU 打配合（截屏导入音高空格Retick），轻松解决网易XS扒不准的日文韩文歌曲！ 该功能也可用于与朋友发图分享工程片段，粘贴聆听图片里的旋律

Alt+T 重录（SV独家的AI功能）
- <mark>👍【即时试听模式】音素面板的标题栏 右键双击开启</mark>
    - 播放中右键拖动走带（按住Shift+右键才是菜单） 右键双击重录并播放，中键滚动音素响度，双击进入音高控制点模式！！SV-TGR 编辑进化，一键不落
    - [12qw] 快捷重录，Shift+2=重录并只保留旧音素，音质更清爽。 🎯1=拉开重录菜单 2=仅音素时值 q=仅音高线 以此类推
    - 重启电脑后可能不响应，点开一次.opu文件即可
- 👍 强制单轨多歌手：模板svp含 "音符组视图"蓝绿轨 请右击音符组设置歌声 + "♯  请在下方选择默认声库"紫轨 +深绿BPM+浅绿VOC 。Alt+G 将强制音轨[0]所有音符组首尾不叠
- 👍【独唱对照模式】，"Ver1", "Ver2" 轨道在切换时，自动静音旧的轨道。Alt+Del 转而删除第一轨道对应音符选区的音符
...
- 🎯分完组做“瑕疵随心选”：
- 在WebUI打开期间在SV中随处播放，您点选音符的走带位置将在框中标红（记住），您点选的行同样会被SV选中。**🎯随时将标红转为选区-再次点击即清空**。推荐注册 `Alt+左右键` 为快速跳转、`Alt+D/X` 为重置音高和合并音符
    - 闭眼按F键 = 点选走带中的音符！默认勾选：F键暂停（不重置走带）
    - 填词全汉字/全平假名自动免空格分词，自动防歌词溢出（插入即替换），-+'号和br 会跳过显示，方便音高的自由修饰。.号音素暂不可编辑，由"*"占位。
    - 👍溢出有按钮进行【重敲段落字数】：选中音符预览音高，未选的自动删除，一行内左右反复选N次可补满N条，之后可Alt+R重敲


**先确立音高f0，听着原曲卡点过t0t1，最后才是填词L+音素ae（吐字与演绎的精调）。左右横跳式的输入卡点和音高？同时弹和唱？那不符合人体工学。**

次要功能
- 【歌词】面板，无需选中音符，速览、速改单行歌词。自动高亮走带中的音符组！
- 【齐唱】面板，分离选中音符创建2~3个音高/时间有偏移的音符组链接（下方或新加名为"H1~3"的轨道）
- 【段落先后对照播放】菜单，支持添加 "BPM-" "VOC-" 开头的两个采样轨，以音符组为单位，交替播放原曲和声/翻唱和声（切换 双Solo vs Voc Mute），方便调声晒
- 调试细节
    - 右上角，可勾选音色三角波（或吉他/WebMIDI合成器）、自动 sil2line 音符组内插入呼吸（的断句阈值）、sil2dots (..|。。) 分段所需的毫秒
...
- Credits:
    ...
    - [SV1/SV2脚本仓库 by 尊贵的阿昆达(akd)](https://space.bilibili.com/12131593/lists/2087283)
...
建议试用：链接音符组取代参数同步模式、 `[', -, br] 中日文字符填词，[+] 切分英文单词`、[免安装MIDI编辑器](https://signalmidi.app/edit)、[免安装跨引擎转换](https://soulmelody.github.io/libresvip-pwa/)、[QQ/云村作为翻唱母带](https://demo.unlock-music.dev/)（单曲4分钟母带为100M）

推荐安装：🧐 ASIO4ALL 192k超清音质、DiffSinger “声线的自由”、UVR5+ytdlp+一键转svp脚本

该svp脚本有在文件窗口中F2键 <mark>【转换/从BV号和mp3新建/保存】翻调项目</mark> 的三大功能，非常直观，最适合多引擎调教师，见下章。

代码调试的特有功能：

- 👍插件页F12可调用 SV API，通过剪贴板监听+eval实现补齐和对象log等，<mark>可视化和改写所选音符的JSON属性</mark>
...
- 点赞截图识别
    ...
    - png2mid用dfs卷积核定位左上角(y轴-y第一音符)/25=音高

以上，为TGR软件设计的【离线可用部分】完整prompt。致敬：[2020年的Hachiko.pygame项目](https://pypi.org/project/hachiko-bapu/)
...
- <https://openvpi.github.io/oputau> 曲库WebUI
    ...
    - 🎯WebSV免安装歌曲查看（注：仅日语Saki声库。本项目与D社官方并无关系）
- GitHub开源svp仓库（可搜索lrc）
    - 双首字母排名，如 `"Let it Go"->/l/i/Let-it-Go/m.svp.opu, "牵丝戏"->/Q/S/牵丝戏/m.ustx.opu, "千本桜"->/-SE/-N/千本桜/m.{vpr.opu,lrc,tag}`
...
- _不支持上传伴奏！！若伴奏来源于BV号，TGR插件自带有【F2脚本一键翻调或转码】_
    ...
    - 👍 复制任何mp3链接（如 <https://www.myfreemp3.com.cn/?page=audioPage&type=netease&name=> 曲库）后，右键+W 只输入曲名也可以。您还可使用 [ncm-cli](https://music.163.com/st/ncmcli)
    - 同理可转码 `"任意文件.mp4.m4a.opu"` 等等。工程互转时，自动把SV的br，ustx不识别的汉字假名转换好
        - .ncm 文件夹下可以回车扒谱并且提示您可能需要词曲授权，但转换时会提示必须买单曲。本项目不涉及视频下载等等 🤗
        - Demucs 可对吉他/钢琴单独分离，[配合这个扒谱！](https://basicpitch.spotify.com/)
    - [🎁 LibreSVIP.exe](https://github.com/SoulMelody/LibreSVIP/releases)
    - [🎁 点此安装1.6G的UVR人声分离器](https://github.com/Anjok07/ultimatevocalremovergui/releases/tag/v5.6)：16G配置的电脑大概是“1.5x倍速播放”吧。其中MDX最快，VRA质量合适，Demucs对合声支持更好
...
上传1个原曲不重复的svp，即得 `@闲_态度/Vocal2Midi` 的1次调用机会（<240s的中文BV视频，公共算力池，群友的PC从 CF Serverless “抢单”。若本机安装GUI则需下载5G模型权重）
